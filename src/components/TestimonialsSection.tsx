        ...
        <div className="relative max-w-4xl mx-auto">
          <div className="overflow-hidden rounded-3xl">
            ...
            transform: `translateX(-${currentSlide * 100}%)`
              ...
              {testimonials.map(testimonial => <div key={testimonial.id} className="w-full flex-shrink-0">
                    ...
                    <CardContent className="p-8 md:p-12 text-center">
                      ...
                      <div className="flex items-center justify-center space-x-4">
                        ...
                        <div className="text-left">
                          ...
                          {testimonial.role && <div className="text-gold font-medium">
                              {testimonial.role}
                            </div>}
          ...
          {/* Navigation Buttons */}
          <Button variant="outline" size="icon" onClick={prevSlide} className="absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-1/2 w-12 h-12 rounded-full bg-white shadow-elegant border-primary/20 hover:border-primary/40">
            <ChevronLeft className="w-6 h-6 text-primary" />
          </Button>

          <Button variant="outline" size="icon" onClick={nextSlide} className="absolute right-0 top-1/2 transform -translate-y-1/2 translate-x-1/2 w-12 h-12 rounded-full bg-white shadow-elegant border-primary/20 hover:border-primary/40">
            <ChevronRight className="w-6 h-6 text-primary" />
          </Button>

          {/* Dots Indicator */}
          <div className="flex justify-center mt-8 space-x-2">
            {testimonials.map((_, index) => <button key={index} onClick={() => setCurrentSlide(index)} className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentSlide ? 'bg-gold scale-125' : 'bg-primary/30 hover:bg-primary/50'}`} />)}
          </div>
        </div>
        ...
        <div className="mt-20 bg-primary rounded-3xl p-8 md:p-12 text-white">
          <div className="grid md:grid-cols-3 gap-8 text-center">
            ...
            <div>
              ...
              <div className="text-xl font-semibold mb-1">Anos</div>
              <div className="text-primary-foreground/80">de experiência de mercado</div>
            </div>
            <div>
